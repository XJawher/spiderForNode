...
/**
    { province: '福建', cityEN: 'ly', cityCH: '龙岩' },
        { province: '福建', cityEN: 'zhangzhpou', cityCH: '漳州' },
 *  { province: '安徽', cityEN: 'cz', cityCH: '滁州' },
 */
const model = {
    async lianjia() {
        let total = '';
        ...
        /*   if (errorCity.length > 0) {
              for (let index = 0; index < errorCity.length; index++) {
                  setTimeout(async () => {
                      await superagent.get(`https://${errorCity[i].cityEN}.lianjia.com/ershoufang/pg1/`).end(async (err, res) => {
                          let $ = null;
                          try {
                              $ = await cheerio.load(res.text);
                          } catch (error) {
                          ...
                          if (err) {
                              ...
                              errorDataInsert.push({
                                  province: errorCity[i].province, cityCH: errorCity[i].cityCH, cityEN: errorCity[i].cityEN,
                                  total: total, time: new Date()
                              });
                              console.log(`第${i}次请求,数据源为${errorCity[i].province}省,${errorCity[i].cityCH}市的数据,总量为:${total},总共有${errorCity.length}个城市`);
                              if (i === cityTotal.length - 1) {
                                  console.log(`出错后再次请求插入数据库数据为: ${errorDataInsert.length}`);
                                  await database.city(errorDataInsert)
                              }
                          }
                      });
                  }, 2000 * i)
