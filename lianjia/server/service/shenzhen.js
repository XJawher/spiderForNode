...
const model = {
    async lianjia() {
        ...
        await superagent.get(`https://sz.lianjia.com/ershoufang/pg1/`).end(async (err, res) => {
            let $ = null;
            try {
        ...
        for (let [key, value] of Object.entries(Region)) {
            if (value.length === 0) {
                for (let i = 0; i < 100; i++) {
                    (function () {
                        setTimeout(async () => {
                            await superagent.get(`https://sz.lianjia.com/ershoufang/${key}/pg${i + 1}/`).end(async (err, res) => {
                                let $ = null;
                                try {
                                    $ = await cheerio.load(res.text);
                    ...
                    (function () {
                        setTimeout(async () => {
                            value.forEach(async (item) => {
                                await superagent.get(`https://sz.lianjia.com/ershoufang/${key}/${item}/pg${i + 1}/`).end(async (err, res) => {
                                    let $ = null;
                                    try {
                                        $ = await cheerio.load(res.text);
